<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input style="width: 150px;" class="filter-item" placeholder="驾驶员"></el-input>
      <el-input style="width: 220px;" class="filter-item" placeholder="证号"></el-input>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" @row-dblclick="handleDblclickRow" @current-change="handleCurrentChangeRow" stripe border fit highlight-current-row
      ...
      <el-table-column min-width="110px" label="驾驶员">
        <template slot-scope="scope">
          <span></span>
        </template>
      </el-table-column>
      <el-table-column min-width="110px" label="证号">
        <template slot-scope="scope">
          <span></span>
        </template>
      </el-table-column>
      <el-table-column min-width="110px" label="准驾车型">
        <template slot-scope="scope">
          <span></span>
        </template>
      </el-table-column>
      <el-table-column min-width="110px" label="有效期">
        <template slot-scope="scope">
          <span></span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleCheck() {
      this.$router.push({ name: 'driverCheck', query: { id: this.currentRow.id }})
    },
    handleDetails() {
      this.$router.push({ name: 'driverDetails', query: { id: this.currentRow.id }})
    }
  }
}
</script>
