<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="包装物名称" clearable v-model.trim="listQuery.name"> </el-input>
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading" @current-change="handleCurrentChangeRow" stripe border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="200px" align="center" label="包装物编号">
        <template slot-scope="scope">
          <span>{{scope.row.code}}</span>
        </template>
      </el-table-column>
      <el-table-column width="200px" align="center" label="包装物名称">
        <template slot-scope="scope">
          <span>{{scope.row.name}}</span>
        </template>
      </el-table-column>
      <el-table-column width="200px" align="center" label="包装物型号">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :before-close="cancel">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="包装物编号" prop="code">
          <el-input v-model.trim="form.code" placeholder="请输入包装物编号"></el-input>
        </el-form-item>
        <el-form-item label="包装物型号" prop="model">
          <el-input v-model.trim="form.model" placeholder="请输入包装物型号"></el-input>
        </el-form-item>
        <el-form-item label="包装物名称" prop="name">
          <el-input v-model.trim="form.name" placeholder="请输入包装物名称"></el-input>
        </el-form-item>
...
<script>
  ...
  import { page, addObj, getObj, delObj, putObj } from '@/api/base/packaging'
    ...
    data() {
      return {
        ...
        rules: {
          ...
          code: [
            { required: true, message: '请输入包装物编号', trigger: 'blur' },
            { min: 2, max: 20, message: '长度在 2 到 20 个字符', trigger: 'blur' }
          ],
          model: [
            { required: true, message: '请输入包装物型号', trigger: 'blur' },
            { min: 2, max: 20, message: '长度在 2 到 20 个字符', trigger: 'blur' }
          ],
          name: [
            { required: true, message: '请输入包装物名称', trigger: 'blur' },
            { min: 2, max: 20, message: '长度在 2 到 20 个字符', trigger: 'blur' }
          ]
        },
        list: null,
